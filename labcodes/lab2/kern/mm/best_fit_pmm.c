...
best_fit_init_memmap(struct Page *base, size_t n) {
    ...
    for (; p != base + n; p ++) {
        ...
        /*LAB2 EXERCISE 2: 2314092 2312483 2311591*/ 
        // 清空当前页框的标志和属性信息，并将页框的引用计数设置为0
p->flags=0;//清空标志
p->property=0;//非块首页的property设为0
...
//按照地址升序插入空闲链表
if(base<page){//找到第一个地址比base大的页
list_add_before(le,&(base->page_link));//插在它的前面
break;
}else if(list_next(le)==&free_list){//遍历到链表尾
list_add(le,&(base->page_link));//插在尾部
}
