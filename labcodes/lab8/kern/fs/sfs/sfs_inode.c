...
sfs_io_nolock(struct sfs_fs* sfs, struct sfs_inode* sin, void* buf, off_t offset, size_t* alenp, bool write) {
    ...
    *alenp = 0;
    ...
    //LAB8:EXERCISE1 2314092 2312483 2311591 HINT: call sfs_bmap_load_nolock, sfs_rbuf, sfs_rblock,etc. read different kind of blocks in file
      /*
    ...
    while (cur < endpos) {
        uint32_t blkno = cur / SFS_BLKSIZE;
        ...
        uint32_t disk_blkno;
        if ((ret = sfs_bmap_load_nolock(sfs, sin, blkno, &disk_blkno)) != 0) {
            break;
        }

        if ((ret = sfs_buf_op(sfs, buf, size, disk_blkno, blkoff)) != 0) {
            break;
        }
