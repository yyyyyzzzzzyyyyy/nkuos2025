...
alloc_proc(void)
{
    ...
    if (proc != NULL)
    {
        ...
        //LAB8 2314092 2312483 2311591 : (update LAB6 steps)
        /*
         * below fields(add in LAB6) in proc_struct need to be initialized
...
void proc_run(struct proc_struct *proc)
{
    ...
    //LAB8 2314092 2312483 2311591 : (update LAB4 steps)
      /*
       * below fields(add in LAB6) in proc_struct need to be initialized
...
int do_fork(uint32_t clone_flags, uintptr_t stack, struct trapframe *tf)
{
    ...
    ret = -E_NO_MEM;
    // LAB8:EXERCISE2 2314092 2312483 2311591  HINT:how to copy the fs in parent's proc_struct?
    // LAB4:填写你在lab4中实现的代码
    /*
     * Some Useful MACROs, Functions and DEFINEs, you can use them in below implementation.
     * MACROs or Functions:
...
{
    /* LAB8:EXERCISE2 2314092 2312483 2311591  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
    ...
    /* (3.2) load program headers */
    struct proghdr ph;
    off_t phoff = elf.e_phoff;

    for (int i = 0; i < elf.e_phnum; i++, phoff += sizeof(ph)) {
    ...
    if ((ret = mm_map(mm, USTACKTOP - USTACKSIZE,
        USTACKSIZE, vm_flags, NULL)) != 0) {
        goto bad_cleanup_mmap;
    }

    for (int i = 1; i <= 4; i++) {
        if (pgdir_alloc_page(mm->pgdir,
            USTACKTOP - i * PGSIZE, PTE_U | PTE_V | PTE_R | PTE_W) == NULL) {
            ret = -E_NO_MEM;
            goto bad_cleanup_mmap;
        }
    }
    ...
    uintptr_t* uargv = kmalloc(sizeof(uintptr_t) * argc);
    if (uargv == NULL) {
        ret = -E_NO_MEM;
        goto bad_cleanup_mmap;
    }
    ...
    sp -= sizeof(uintptr_t) * argc;
    memcpy((void*)sp, uargv, sizeof(uintptr_t) * argc);
